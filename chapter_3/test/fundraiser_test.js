...
contract("Fundraiser", (accounts) => {
  ...
  describe("initalization", () => {
    ...
    it("gets the owner", async () => {
      ...
      assert.equal(actual, owner, "custodian should match");
    });
  });
  ...
  describe("withdrawing funds", () => {
    ...
    it("transfers balance to beneficiary", async () => {
      ...
      const newCoontractBalance = await web3.eth.getBalance(fundraiser.address);
      const newBeneficiaryBalance = await web3.eth.getBalance(beneficiary);
      const beneficiaryDifference =
        newBeneficiaryBalance - currentBeneficiaryBalance;

      assert.equal(newCoontractBalance, 0, "contract should have a 0 balance");
      assert.equal(
